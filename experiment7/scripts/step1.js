let lottie1 = bodymovin.loadAnimation({
    ...
    loop: false,
    autoplay: false,
    path: 'https://assets4.lottiefiles.com/packages/lf20_bSsVzI.json'
})
...
function platySegments1() {
    if (machineOn && !task_done) {
        if (currentSegment1 == 1) {
            ...
            var obj = {i:0}
            gsap.to(obj, {delay: 6, i: 1, ease: new SteppedEase.config(1), onUpdate: () => {
                document.querySelector('#step-1 .reading').innerHTML = `${obj.i.toFixed(1)} gms`
            }})
        }
            ...
            var obj = {i:1}
            gsap.to(obj, {delay: 6, i: 2, ease: new SteppedEase.config(1), onUpdate: () => {
                document.querySelector('#step-1 .reading').innerHTML = `${obj.i.toFixed(1)} gms`
            }})
        }
